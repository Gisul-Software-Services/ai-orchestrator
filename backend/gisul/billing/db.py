...
def aaptor_orgs():
    """Read-only: Aaptor organization collection."""
    s = get_settings()
    return get_client()[s.organization_db_name]["organizations"]
...
async def ensure_indexes() -> None:
    ...
    await usage_logs().create_index(
        [("org_id", 1), ("route", 1), ("billing_period", -1)]
    )

...
export default function AimlLibraryPlaygroundPage() {
    ...
    async (e: React.FormEvent) => {
      ...
      try {
        const resp = await adminPostJson<{ job_id: string }>(
          "/api/admin/generate/aiml-library",
          payload
        );
  ...
  const handleComplete = useCallback(
    (res: unknown) => {
      ...
      setSubmitting(false);
      setJobId(null);
      setHistory((h) => [{ timestamp: ts, endpoint: "AIML (library)", payload, result: res, durationSeconds: dur }, ...h].slice(0, 10));
      setSelectedIdx(0);
    },
    [payload, t0]
  );
  ...
  return (
    <div className="space-y-4">
      <EndpointFormShell
        title="AIML — Library Dataset"
        description="Generate AIML problems grounded in real datasets from the catalog."
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          <Field label="Topic (required)">
            <TextInput
              ...
              required
              placeholder="e.g. iris classification, customer churn"
            />
          </Field>
          ...
          <Field label="Concepts (comma separated)" hint="Optional">
            <TextInput
              ...
              placeholder="e.g. preprocessing, evaluation, feature selection"
            />
          </Field>
        </div>

...
export default function AimlSyntheticPlaygroundPage() {
  ...
  const payload = useMemo(() => {
    const conceptsList = concepts
      ...
      .map((s) => s.trim())
      .filter(Boolean);
    // Backend model for synthetic AIML only requires topic/difficulty/use_cache.
    // We keep concepts in the payload (ignored if backend doesn't accept extras).
    return {
      topic: topic.trim(),
      difficulty,
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          <Field label="Topic (required)">
            <TextInput
              ...
              required
              placeholder="e.g. imbalanced classification with SMOTE"
            />
          </Field>
          ...
          <Field label="Concepts (comma separated)" hint="Optional">
            <TextInput
              ...
              placeholder="e.g. scaling, leakage, evaluation"
            />
          </Field>
        </div>

...
export default function CodingPlaygroundPage() {
  ...
  const payload = useMemo(() => {
    ...
    return {
      ...
      use_cache: useCache,
      // backend expects these fields; keep sane defaults for admin testing
      job_role: "Software Engineer",
      experience_years: "3-5 (Mid-level)",
    };
  }, [topic, difficulty, numQuestions, language, concepts, useCache]);
  ...
  const handleComplete = useCallback(
    (res: unknown) => {
      ...
      setSubmitting(false);
      setJobId(null);
      setHistory((h) => [{ timestamp: ts, endpoint: "Coding", payload, result: res, durationSeconds: dur }, ...h].slice(0, 10));
      setSelectedIdx(0);
    },
    [payload, t0]
  );
  ...
  return (
    <div className="space-y-4">
      <EndpointFormShell
        title="Coding problem generation"
        description="Generate coding problems with starter code."
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          <Field label="Topic (required)">
            <TextInput
              ...
              required
              placeholder="e.g. two pointers, graph traversal"
            />
          </Field>
          ...
          <Field label="Concepts (comma separated)" hint="Optional">
            <TextInput
              ...
              placeholder="e.g. bfs, dfs, hashmap"
            />
          </Field>
        </div>

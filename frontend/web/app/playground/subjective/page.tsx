...
export default function SubjectivePlaygroundPage() {
  ...
  const payload = useMemo(() => {
    ...
    return {
      ...
      use_cache: useCache,
      // backend requires target_audience
  ...
  const handleComplete = useCallback(
    (res: unknown) => {
      ...
      setSubmitting(false);
      setJobId(null);
      setHistory((h) => [{ timestamp: ts, endpoint: "Subjective", payload, result: res, durationSeconds: dur }, ...h].slice(0, 10));
      setSelectedIdx(0);
    },
    [payload, t0]
  );
  ...
  return (
    <div className="space-y-4">
      <EndpointFormShell
        title="Subjective generation"
        description="Generate subjective questions and sample answers."
        ...
        <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
          ...
          <Field label="Concepts (comma separated)" hint="Optional">
            <TextInput
              ...
              placeholder="e.g. hooks, rendering, memoization"
            />
          </Field>
        </div>
      ...
      {result && resultTs ? (
        <ResponseCard endpoint="Subjective" result={result} timestamp={resultTs} durationSeconds={t0 ? (resultTs - t0) / 1000 : undefined} />
      ) : null}
